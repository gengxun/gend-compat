...
class Gend_Log extends Gend
{
	 ...
	 * @return void
	 **/
	public static function write($msg, $file = 'Gend.log', $debug = 0)
	{
		// 将数据转换为字符串
		...
		// 写入日志
        if(!file_exists(SYS_ROOTDIR. "logs/Gend/")){
            mkdir(SYS_ROOTDIR . "logs/Gend/",0777,1);
        }
		file_put_contents(SYS_ROOTDIR . "logs/Gend/" . $file, date("Y-m-d H:i:s > ") . "{$msg}\n", FILE_APPEND);
	}
	...
	private static function recordLog($logname, $tag, $file, $line, $msg, $extra = array())
	{
		...
		$log = array(
			"x_name" => $logname,
			"x_trace_id" => Gend_EagleEye::getTraceId(),
			"x_rpc_id" => Gend_EagleEye::getCurrentRpcId(),
			"x_version" => Gend_EagleEye::getVersion(),
			"x_timestamp" => time(),
			"x_module" => $tag,
			"x_uid" => Gend_EagleEye::getRequestLogInfo("uid"),
			"x_pid" => getmypid(),
