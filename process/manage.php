...
class Gend_Process_manage  extends  Gend
{
    ...
    public  function  monitorStart()  {
        ...
        while(1) {
            ...
            if ($ret) {
                ...
                $pid    = $ret['pid'];
                Gend_Log::write("monitor exit, pid=" . $pid . ",restart it!",$this->config['logName']);
                $this->pid = $this->newMonitor();
            }
            sleep(1);
        }
...
//                Gend_Log::write($tableCount);
                if($tableCount  <  $this->config['processNum'])  {
                    Gend_Log::write('monitor  new  process',$this->config['logName']);
                    for  ($i  =  0;$i  <  $this->config['processNum']  -  $tableCount;  ++$i)  {
                        $info  =  $this->registeProcess($this->config['name']);
                        if(!$info)  {
                            Gend_Log::write('start  work  process  faild,please  check  memory or swoole\'s log',$this->config['logName']);
                        }
                    }
                }
                else   {
        ...
        Gend_Log::write('start monitor process',$this->config['logName']);
        $pid = $process->start();
        if(!$pid) {
            Gend_Log::write('start  monitor  process  faild,please  check  memory or swoole\'s log',$this->config['logName']);
            return false;
        }
        return $pid;
    }
    ...
    private function killAllProcess() {
        Gend_Log::write('kill all process',$this->config['logName']);
        foreach ($this->table as $index => $item) {
            swoole_process::kill($item['pid']);
        }
        Gend_Log::write('kill monitor',$this->config['logName']);
        exit(0);
    }
    ...
    private function killMonitor() {
        Gend_Log::write('kill monitor process',$this->config['logName']);
    ...
    public function childDie() {
        while($ret = swoole_process::wait(false)) {
            ...
            Gend_Log::write('a work process has been gone,wait monitor restart',$this->config['logName']);
            return true;
        }
    }
    ...
    private function killMe() {
        ...
        $this->table->del($key);
        $this->exiting();
        Gend_Log::write('work process has been killed,pid is '.$pid,$this->config['logName']);
        exit;
    }
    ...
    private function dumpTable() {
        ...
        Gend_Log::write(json_encode($statics));
    }
